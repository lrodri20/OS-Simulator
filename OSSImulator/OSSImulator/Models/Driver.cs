...
namespace OSSImulator.Controllers
{
    public class Driver
    {
        ...
        public static Task<int>[] cpuFutures;
        ...
        public static void fire()
        {
            ...
            {
                if (cpuFutures[i] != null && cpuFutures[i].IsCanceled)
                {
                    Console.WriteLine("XXXXXXx CANCELLED XXXXX" + i);
                    try
                    {
                        if (cpuFutures[i] != null && cpuFutures[i].IsCompleted)
                        {
                            //cpuFutures[i].;
                        }
                    }
                    ...
                    if (((cpuFutures[i] == null || cpuFutures[i].IsCompleted || cpuFutures[i].IsCanceled) && CPUs[i].isJobLoaded() && !CPUs[i].shouldUnload()))
                    {
                        //cpuFutures[i] = ThreadPool.(CPUs[i]);
                            //executorService.submit(CPUS[i]);
